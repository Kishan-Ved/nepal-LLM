...
def extract_data_from_html(html_content):
    try:
        ...
        classes = soup.find_all('div', {'class': 'row news-detail-content justify-content-end'})
        for c in classes:
            p_tags = c.find_all('p')
            extracted_data = ""
            for p in p_tags:
        ...
        print(f"An error occurred while extracting Nepali data: {e}")

    return extracted_data

def process_link_and_write(index, link, text_file_path):
    html_content = crawl_data_from_link_with_retry("https://farakdhar.com" + link)
    if html_content:
...
csv_path = "./farakdhar.csv"

df = pd.read_csv(csv_path)

if __name__ == "__main__":
    output_folder = "./content/farakdhar"
    failed_csv_path = "./failed_links_farakdhar.csv"
